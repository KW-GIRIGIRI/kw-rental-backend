...
public class ItemRepositoryImpl implements ItemRepository {
	...
	public int saveAll(List<Item> items) {
		itemConstraintPolicy.validateNotDeletedPropertyNumberIsUnique(
			items.stream().map(Item::getPropertyNumber).toList());
	...
	public int updatePropertyNumbers(final List<ToBeUpdatedItem> toBeUpdatedItems) {
		final List<String> propertyNumbers = toBeUpdatedItems.stream()
			.map(ToBeUpdatedItem::toBePropertyNumber)
			.toList();
		itemConstraintPolicy.validateNotDeletedPropertyNumberIsUnique(propertyNumbers);
		return itemJpaRepository.updatePropertyNumbers(toBeUpdatedItems);
	}

	@Override
	public List<Item> findByIds(final Collection<Long> ids) {
		return itemJpaRepository.findByIds(ids);
	}
}
