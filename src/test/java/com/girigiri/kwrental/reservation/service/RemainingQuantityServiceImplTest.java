...
class RemainingQuantityServiceImplTest {
    ...
    void validateAmount() {
        ...
        final RentalSpec rentalSpec1 = RentalSpecFixture.builder(equipment).amount(new RentalAmount(1))
                .period(new RentalPeriod(LocalDate.now(), LocalDate.now().plusDays(1))).build();
        final RentalSpec rentalSpec2 = RentalSpecFixture.builder(equipment).amount(new RentalAmount(1))
        ...
        given(rentalSpecRepository.findOverlappedByPeriod(any(), any())).willReturn(List.of(rentalSpec1, rentalSpec2));
