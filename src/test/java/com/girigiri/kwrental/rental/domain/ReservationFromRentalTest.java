...
class ReservationFromRentalTest {
	...
	void nowIsLegalForReturn() {
		...
		final ReservationSpec reservationsSpec1 = ReservationSpecFixture.builder(null)
			.id(1L)
		...
		final ReservationSpec reservationsSpec2 = ReservationSpecFixture.builder(null)
			.id(2L)
	...
	void setStatusAfterReturn_returned() {
		...
		// then
		reservationFromRental1.setStatusAfterReturn(
			Map.of(reservationSpec1.getId(), List.of(RentalSpecStatus.RETURNED, RentalSpecStatus.RETURNED)));
	...
	void setStatusAfterReturn_abnormalReturned(final RentalSpecStatus abnormalReturnedStatus) {
		...
		// then
		reservationFromRental1.setStatusAfterReturn(
			Map.of(reservationSpec1.getId(), List.of(RentalSpecStatus.RETURNED, abnormalReturnedStatus)));
	...
	void setStatusAfterReturn_overdueRented(final RentalSpecStatus abnormalReturnedStatus) {
		...
		// then
		reservationFromRental1.setStatusAfterReturn(Map.of(reservationSpec1.getId(),
			List.of(RentalSpecStatus.RETURNED, abnormalReturnedStatus, RentalSpecStatus.OVERDUE_RENTED)));
	...
	void setStatusAfterReturn_rentedException() {
		...
		assertThatThrownBy(() ->
			reservationFromRental1.setStatusAfterReturn(
				Map.of(reservationSpec1.getId(), List.of(RentalSpecStatus.RETURNED, RentalSpecStatus.RENTED)))
